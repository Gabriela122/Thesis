<template>
  <div id="home">
    <div id="home-cont">
      <img src="../assets/zdj.jpg" alt="zdj1" class="zdj" />
      <div class="text">
        <h1 class="header1">Lorem Ipsum</h1>
        <p class="paragraph1">
          Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer varius dictum metus
          quis aliquet. Cras ac convallis nisl, venenatis porttitor odio. Integer dictum est eget tortor
          tristique, et fringilla enim vulputate. Nulla facilisi. Aliquam erat volutpat. Morbi ut lacus ac nisi
          efficitur tristique. Quisque interdum ex ac luctus aliquet. Aenean viverra sem luctus, malesuada nisi
          sed, auctor massa. Maecenas pretium vulputate sapien ut semper. Nulla eget purus ornare, finibus velit
          eget, elementum orci. Lorem ipsum dolor sit amet, consectetur adipiscing elit.
        </p>
      </div>
    </div>
    <a href="#container1">
      <div class="arrow">
        <p class="arrow-para">V</p>
      </div>
    </a>
  </div>
</template>

<script>
export default {
  name: "Home",
};
</script>
